...
import {SWEEP, FLAG, UNFLAG, SET_HINT, CLEAR_HINTS} from 'actions/Game';
...
const reveal = (board, id) => {
  ...
  if (
    board[id.x][id.y].number > 0 &&
    board[id.x][id.y].sweeped &&
    !board[id.x][id.y].isMine
  ) {
    const neighbors = notRevealedNeighbors(board, id);
    const correctItems = [];
    neighbors.forEach(neighbor => {
      if (
        board[neighbor.x][neighbor.y].isMine &&
        board[neighbor.x][neighbor.y].flagued
      ) {
    ...
    if (
      ...
      correctItems.length === board[id.x][id.y].number
    ) {
      const theRestNeighbors = neighbors.filter(
        neighbor =>
          !correctItems.some(
            item => neighbor.x === item.x && neighbor.y === item.y,
          ),
      );
